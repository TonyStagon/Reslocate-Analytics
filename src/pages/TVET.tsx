...
export function TVET() {
  ...
  const filteredColleges = useMemo(() => {
    return colleges.filter(college => {
      if (selectedCollege !== 'all' && college.tvet_college_name !== selectedCollege) return false
      if (selectedFaculty !== 'all' && college.faculty !== selectedFaculty) return false
      if (selectedAPS !== 'all') {
        const aps = college.aps
        switch (selectedAPS) {
          case '0-15': return aps <= 15
          case '16-20': return aps >= 16 && aps <= 20
          case '21-25': return aps >= 21 && aps <= 25
          case '26+': return aps >= 26
          default: return true
        }
      }
      return true
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        {(selectedCollege !== 'all' || selectedFaculty !== 'all' || selectedAPS !== 'all') && (
          <button
            onClick={() => {
      ...
      <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
        <p className="text-sm text-gray-600">
          Showing <span className="font-semibold text-gray-900">{filteredColleges.length}</span> of{' '}
          <span className="font-semibold text-gray-900">{colleges.length}</span> programs
        </p>
      </div>
